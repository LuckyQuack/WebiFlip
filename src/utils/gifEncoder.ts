...
const toRgbaFrameData = (frame: ImageData | null, width: number, height: number): Uint8Array => {
  ...
  for (let i = 0; i < pixelCount; i++) {
    ...
    const inv = 1 - alpha;
    rgba[offset] = Math.round(frame.data[offset] * alpha + 255 * inv);
    rgba[offset + 1] = Math.round(frame.data[offset + 1] * alpha + 255 * inv);
    rgba[offset + 2] = Math.round(frame.data[offset + 2] * alpha + 255 * inv);
    rgba[offset + 3] = 255;
  }
...
export const encodeGif = ({ width, height, frames, fps = 6, loop = true }: EncodeGifOptions): Uint8Array => {

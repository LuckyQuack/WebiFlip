...
import type { BoardPost, CreateBoardPostOptions } from '../types';
...
const normalizePost = (row: Record<string, unknown>): BoardPost => ({
  id: String(row.id),
  title: String(row.title || 'Untitled flip'),
  caption: String(row.caption || ''),
  author: String(row.author || ''),
  gifUrl: String(row.gif_url || row.gifUrl || ''),
  posterFrameUrl: String(row.poster_frame_url || row.posterFrameUrl || ''),
...
const withSetupHint = (error: { message?: string } | null, fallbackMessage: string): string => {
...
export const listBoardPosts = async ({ limit = 120 } = {}): Promise<BoardPost[]> => {
  requireSupabase();
...
export const createBoardPost = async ({
  ...
  fps,
  frameCount,
}: CreateBoardPostOptions): Promise<BoardPost> => {
  requireSupabase();
  ...
  if (uploadError) {
    throw new Error(
      withSetupHint(uploadError, `Could not upload the GIF to the "${BOARD_GIFS_BUCKET}" bucket. ${uploadError.message}`)
    );
  }
  ...
  if (error) {
    throw new Error(
      withSetupHint(error, `Could not save the board post in "${BOARD_POSTS_TABLE}". ${error.message}`)
    );
  }

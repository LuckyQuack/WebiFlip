...
const BoardPage = ({ refreshToken, onNavigateToEditor }: BoardPageProps) => {
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Could not load the board right now.');
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => { loadPosts(); }, [refreshToken]);

  const handleDownloadSelectedPost = async () => {
    if (!selectedPost?.gifUrl || isDownloadingGif) return;

    setIsDownloadingGif(true);
    try {
      const response = await fetch(selectedPost.gifUrl);
      if (!response.ok) throw new Error(`Download failed with status ${response.status}.`);
      downloadBlob(await response.blob(), getGifExportFileName(selectedPost.title || 'webiflip-board-post'));
    } catch (error) {
  ...
  return (
    <main className="board-page">
      ...
      {errorMessage ? (
        <section className="board-status-panel"><p>{errorMessage}</p></section>
      ) : null}

      {!errorMessage && isLoading ? (
        <section className="board-status-panel"><p>Loading board posts...</p></section>
      ) : null}
